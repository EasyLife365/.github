...
//
// DECISION, recorded here per EasyLife365/.github#96's own acceptance criteria: the PR-metadata
// checks (pr-title, branch-name) are SKIPPED on merge_group, explicitly and visibly (a notice
// naming why), not silently passed. A prior version of this file re-fetched the pull request's
// current title/body over the API instead to close the small TOCTOU gap that skipping leaves
// (a body edit after queue admission isn't re-checked) -- reverted (EasyLife365/.github#127)
// because the extra pull-requests: read permission it needed broke workflow startup for every
// caller that hadn't also been updated to grant it, taking down BOTH required checks (not just
// this one) org-wide. Redo the re-fetch only alongside a coordinated caller-side permissions
// rollout, not as a same-file change here.
...
const baseSha = pr ? pr.base?.sha : mergeGroup.base_sha;
const headSha = pr ? pr.head?.sha : mergeGroup.head_sha;
const author = pr ? pr.user?.login || '' : '';
const prNumber = pr ? pr.number : PR_NUMBER_IN_REF.exec(mergeGroup.head_ref || '')?.[1];
...
console.log(
  pr
    ...
    : `Merge group for pull request #${prNumber ?? '?'} (${mergeGroup.head_ref})`,
);
